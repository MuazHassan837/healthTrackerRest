...
class HealthTrackerControllerTest {
    ...
    inner class ReadHydrationObj {
        ...
        fun `read non-existing hydration list return 404`(){
            assertEquals(getAllInTakes().status,404)
        }


        @Test
        fun `read Intake for user id when it doesnt exists return 404`(){
            assertEquals(404, getInTakeForUserID(Int.MIN_VALUE).status)
        }
        ...
        fun `read intake for user id when it exists but intake list doesnt returns 404`(){
            val newUser : User = jsonToObject(addUser(validID,validName, validEmail).body.toString())
            assertEquals(404, getInTakeForUserID(newUser.id).status)
            assertEquals(204,deleteUser(newUser.id).status)
        }
        ...
        fun `read invalid intake with id return 404`(){
            assertEquals(404, getInTakeByID(Int.MIN_VALUE).status)
        }
    }
    ...
    inner class DelHydrationObj {
        ...
        fun `delete intakes of invalid user returns 404`(){
            assertEquals(404,delIntakeByUserID(Int.MIN_VALUE).status)
        }
        ...
        fun `delete intake by id for invalid intake id return 404`(){
            assertEquals(404,delIntakeByID(Int.MIN_VALUE).status)
        }
    ...
    inner class UpdateHydrationObj{
        ...
        fun `update valid Intake by id return 204`(){
            ...
            assertEquals(updatedAmount,convertedFetchedIntake.amountltr)
            assertEquals(updatedSubstance,convertedFetchedIntake.substance)
            assertEquals(204,delIntakeByUserID(newUser.id).status)
            assertEquals(204,deleteUser(newUser.id).status)
        }
