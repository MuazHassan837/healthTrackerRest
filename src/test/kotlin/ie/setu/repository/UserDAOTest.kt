    ...
    inner class createAndTest {
        ...
        fun `multiple users added to table can be retrieved successfully`() {
            transaction {
                ...
                //Act & Assert
                TestCase.assertEquals(3, userDAO.getAll().size)
                TestCase.assertEquals(user1, userDAO.findById(user1.id))
                TestCase.assertEquals(user2, userDAO.findById(user2.id))
                TestCase.assertEquals(user3, userDAO.findById(user3.id))
            }
        }
        @Test
